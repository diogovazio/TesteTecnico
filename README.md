...
Em ter, 9 de abr de 2019 às 22:22, Diogo Nogueira Rocha <diogo.d1bh@gmail.com> escreveu:
*Instalação do Docker*

1- Remover versões anteriores
	sudo apt-get remove docker docker-engine docker.io containerd runc
2- Instalar o Docker 
2.1 Atualizar o repsirotio
	sudo apt-get update
2.2 Atualizar os pacotes e pemritir a utização de reposoti
	$ sudo apt-get install \
    	apt-transport-https \
    	ca-certificates \
    	curl \
    	gnupg-agent \
    	software-properties-common
3.0 Add Docker’s official GPG key:
	$ curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add -
4.0 cnfigurar repositori estavel do docker
	sudo add-apt-repository \
   "deb [arch=amd64] https://download.docker.com/linux/ubuntu \
   $(lsb_release -cs) \
   stable"

5.0 Atualizar os pacotes
	sudo apt-get update
6.0 instalaro docker
	sudo apt-get install docker-ce docker-ce-cli containerd.io
 ** Utilizado codumentação officeial / https://docs.docker.com/install/linux/docker-ce/ubuntu/**


Instalação do composer

1.0 baixar a versão estavel do docker cmposer
	sudo curl -L "https://github.com/docker/compose/releases/download/1.24.0/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose
2.0 Aplicar permição de executavel
	sudo chmod +x /usr/local/bin/docker-compose
...
https://tidahora.com.br/instalando-zabbix-server-4-no-linux-centos-7/




-- 
Diogo Nogueira Rocha
T.I
